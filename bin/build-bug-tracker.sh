...
MODULE_PATH="custom/Espo/Modules/BugTracker"
CLIENT_MODULE_PATH="client/custom/modules/bug-tracker"
...
PACKAGE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bug-tracker-package.XXXXXX")"
...
cp "$ROOT_DIR/bin/packaging/BugTracker-zip-AfterInstall.php" "$PACKAGE_DIR/scripts/AfterInstall.php"

OUTPUT="$ROOT_DIR/dist/bug-tracker-v${VERSION}.zip"
rm -f "$OUTPUT"
