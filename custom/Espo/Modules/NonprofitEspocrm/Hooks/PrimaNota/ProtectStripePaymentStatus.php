...
/**
 * Stripe paymentStatus is UI-read-only and drives Inviato-only dashlets / fundraising.
 * Keep updates available for website ingest (type=api) and SKIP_ALL retries only.
 * Interactive staff must not flip Refunded/Disputed/Inviato via API or mass update.
 */
class ProtectStripePaymentStatus implements BeforeSave
{
    ...
    public function beforeSave(Entity $entity, SaveOptions $options): void
    {
        ...
        // Website ingest uses type=api. System/CLI retries use SKIP_ALL above.
        if ($this->user->isApi()) {
            return;
        }
